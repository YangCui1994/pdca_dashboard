...
Run the local app (on Windows use `python` instead of `python3`):
...
python -m unittest discover -s workbench/tests -p 'test_*.py' -v  # Flet workbench suite (needs `pip install flet`)
```
